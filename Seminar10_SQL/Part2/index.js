...
application.get('/universities/:universityId/courses/:courseId/enrollments',
  async (req,res,next) =>
    {
      try{
      const university=await University.findByPk(request.params.universityId);
      if(university)
        {
          const courses=await university.getCourses({id: request.params.course});
          const course=courses.shift();
          if(course)
            {
...
application.post('/universities/:universityId/courses/:courseId/enrollments/:studentId',
  async (req,res,next) =>
    {
      try{
      const university=await University.findByPk(request.params.universityId);
...
application.delete('/universities/:universityId/courses/:courseId/enrollments/:studentId',
  async (req,res,next) =>
    {
      try{
      const university=await University.findByPk(request.params.universityId);
      if(university)
        {
          ...
          if(course && student)
            {
              course.removeStudentById(student);
              await course.save();
              response.sendStatus(204);
            }
            else response.sendStatus(404);
...
////////////////
application.get("/universities/:universityId/students/:studentId/enrollments",
  async (req,res,next) =>
    {
      try {
