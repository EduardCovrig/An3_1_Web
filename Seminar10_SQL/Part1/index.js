...
app.get("/universities/:universityId/students/:studentId", async (req, res, next) => {
  try {
    ...
    if (university) {
      const student = university.students.find(s => s.id == req.params.studentId); // .filter(s => s.id==req.params.studentId)[0];
      if (student) {
        res.status(200).json(student);
      }
      else res.status(404).json({ "Message": "Student not found" });
    }
    else res.status(404).json({ "Message": "Universtiry not found" });
  }
  catch (err) {
    next(err);
  }
...
app.delete("/students/:studentId", async (req, res, next) => {
  ...
  try {
    student = await Student.findByPk(req.params.studentId);
    if (student) {
